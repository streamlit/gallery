...
def _source_components_dir(repo_root: Path) -> Path:
    ...
    Prefer the current location under `components/registry/components/`. If that
    directory is missing but a legacy `components/` directory exists at repo
    root, fall back to that for backward compatibility.
    """
    ...
    # Default to the current expected path for error messaging.
    return candidate
...
def validate_components(repo_root: Path) -> list[ValidationIssue]:
    ...
    components_dir = _source_components_dir(repo_root)
...
def main(argv: list[str]) -> int:
    ...
    if not components_dir.is_dir():
        print(
            "ERROR: Missing source components directory. Expected one of:\n"
