...
class Enricher(Protocol):
    ...
    def patch_success(self, result: Any, fetched_at: str) -> Patch: ...

    def patch_failure(
        self, comp: dict[str, Any], error: str | None, status: int | None
    ) -> Patch: ...
...
class ThreadLocalSession:
    def __init__(self) -> None:
        ...
        # Track created sessions so we can explicitly close them when a run finishes.
        # This avoids leaving pooled connections hanging around until GC.
        self._lock = threading.Lock()
    ...
    def close_all(self) -> None:
        """Close all sessions created by this ThreadLocalSession."""
        with self._lock:
        ...
        for s in sessions:
            with suppress(Exception):
                s.close()
        # Clear this thread's reference (other thread-locals will die with the threads).
        with suppress(Exception):
...
def run_enrichment_engine(
    *,
    ...
    def submit_fetch(enricher: Enricher, key: Any) -> Future[FetchResult]:
        ...
        def _run() -> FetchResult:
            ...
            try:
                ...
                # Never let exceptions in worker threads crash the entire run.
                return FetchResult(
                    ok=False,
                    data=None,
    ...
    try:
        with ThreadPoolExecutor(max_workers=max(1, int(workers))) as executor:
            ...
            for idx, comp in enumerate(components):
                ...
                for enricher, fut in comp_tasks[idx]:
                    ...
                    if res.ok:
                        patch = enricher.patch_success(res.data, run_fetched_at)
                    else:
                        patch = enricher.patch_failure(comp, res.error, res.status)
                ...
                if progress_every and progress_every > 0 and (idx + 1) % progress_every == 0:
                    for enricher in enricher_list:
        ...
        # Always close pooled connections for deterministic cleanup.
        for s in session_by_service.values():
            s.close_all()
