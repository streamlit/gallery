...
class PyPiStatsEnricher:
    ...
    def needs_fetch(self, comp: dict[str, Any], refresh_older_than_hours: float | None) -> bool:
        ...
        pypistats_metrics = metrics.get("pypistats") if isinstance(metrics, dict) else None
        existing_fetched_at = (
            pypistats_metrics.get("fetchedAt") if isinstance(pypistats_metrics, dict) else None
        )
        stale = pypistats_metrics.get("isStale") if isinstance(pypistats_metrics, dict) else None
    ...
    def patch_success(self, result: PyPiStatsResult, fetched_at: str) -> Patch:
        updates = {
