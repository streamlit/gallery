...
This script compiles per-component submissions in `components/registry/components/*.json` into a
single compatible artifact at `components/registry/compiled/components.json` that the
...
Common variants:
    ...
    python components/registry/scripts/build_catalog.py --out dist/components.json

    # Skip invalid component JSON files (prints errors and continues)
    python components/registry/scripts/build_catalog.py --skip-invalid

    # Explicitly choose the prior artifact used for carry-forward
    python components/registry/scripts/build_catalog.py --previous components/registry/compiled/components.json
"""
...
def _strip_error_fields(bucket: dict[str, Any] | None) -> dict[str, Any] | None:
    if not isinstance(bucket, dict):
        return bucket
    cleaned = dict(bucket)
    cleaned.pop("lastError", None)
    cleaned.pop("lastErrorAt", None)
    cleaned.pop("lastStatus", None)
    return cleaned


def _pip_cmd_from_submission(links: dict[str, Any], install: dict[str, Any] | None) -> str | None:
...
def build_catalog(
    *,
    ...
    for json_file in sorted(components_dir.glob("*.json")):
        try:
            submission = load_json(json_file)
        except json.JSONDecodeError as e:
            errors.append(ComponentBuildError(file=json_file, message=str(e), json_path=None))
            continue
        ...
        if any(e.file == json_file for e in errors):
            continue
        ...
        try:
            ...
            media = submission.get("media") if isinstance(submission.get("media"), dict) else None
            ...
            prev_contributors = _prev_int(prev, "metrics", "github", "contributorsCount")
            ...
            prev_pypi = _strip_error_fields(prev_pypi)
            prev_pypistats = _strip_error_fields(prev_pypistats)
            prev_ranking = prev.get("ranking")
            if not isinstance(prev_ranking, dict):
                # Keep shape stable if previous artifact didn't have rankings.
                prev_ranking = None
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Build the compiled components catalog.")
    parser.add_argument(
        "--out",
        default=None,
        help=(
            "Output path for the compiled JSON "
            "(default: components/registry/compiled/components.json)."
        ),
    )
    parser.add_argument(
        "--components-dir",
        default=None,
        help=(
            "Directory containing per-component JSON submissions "
            "(default: components/registry/components/)."
        ),
    )
    parser.add_argument(
        "--previous",
        ...
        help=(
            ...
            "Defaults to components/registry/compiled/components.json if present."
        ),
    )
    parser.add_argument(
        "--skip-invalid",
    ...
    out_path = Path(args.out) if args.out else (registry_root / "compiled" / "components.json")
    default_components_dir = registry_root / "components"
    components_dir = Path(args.components_dir) if args.components_dir else default_components_dir

    previous_path: Path | None
    if args.previous:
        previous_path = Path(args.previous)
    else:
        candidate = registry_root / "compiled" / "components.json"
    ...
    if errors and not args.skip_invalid:
        print("ERROR: build failed due to invalid component submissions:", file=sys.stderr)
        for e in errors:
