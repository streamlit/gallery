...
def _source_components_dir(repo_root: Path) -> Path:
    """Return the directory containing source-of-truth component submissions.

    Prefer `components/registry/components/`. Fall back to legacy `components/`
    at repo root if needed.
    """
    registry_root = repo_root / "components" / "registry"
    candidate = registry_root / "components"
    if candidate.is_dir():
        return candidate
    legacy = repo_root / "components"
    if legacy.is_dir():
        return legacy
    return candidate
...
def main(argv: list[str]) -> int:
    ...
    components_dir = (
        Path(args.components_dir) if args.components_dir else _source_components_dir(repo_root)
    )
