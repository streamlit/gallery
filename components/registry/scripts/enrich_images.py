...
Typical usage (from repo root):
  python components/registry/scripts/enrich_images.py --check-only
...
def _is_imageish_content_type(ct: str | None) -> bool:
    ...
    if base.startswith("image/"):
        return True
    return base in {"application/octet-stream"}
...
def _check_fetchable(session: requests.Session, url: str, *, timeout_s: float) -> ImageCheck:
    ...
    try:
        with session.head(url, allow_redirects=True, timeout=timeout_s, headers=headers) as r:
...
def _fetch_one(task: _FetchTask, *, timeout_s: float, pool_maxsize: int) -> _FetchResult:
...
def check_images(
    *,
    ...
    ----------
    components_dir
        Directory containing source submission `*.json` files.
    timeout_s
    ...
    for json_name in sorted(local_failures.keys()):
        print(f"[images] FAIL {json_name}: {local_failures[json_name]}", file=sys.stderr)
        failures += 1
    ...
    if tasks:
        ...
        w = max(1, int(workers))
        # This is per-thread (we create one Session per worker thread), so it must NOT scale
        # with `workers` or we end up with O(workers^2) pooled connections.
        #
        # Concurrency is controlled by the ThreadPoolExecutor's max_workers; keep per-thread
        # pool sizes small to avoid FD exhaustion / accidental abuse.
        pool_maxsize = 8
...
def main(argv: list[str]) -> int:
    ...
    """
    parser = argparse.ArgumentParser(description="Validate `media.image` URLs for components.")
    parser.add_argument(
        "--components-dir",
        default=None,
        help=(
    ...
    components_dir = (
        Path(args.components_dir) if args.components_dir else source_components_dir(repo_root)
    )
