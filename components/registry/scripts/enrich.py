...
# NOTE: These scripts are meant to run from within `components/registry/scripts/` (or be
# invoked by sibling scripts that add this directory to `sys.path`). Because this isn't a
# packaged module, static type checkers like mypy may not resolve these local imports.
...
from _utils.github_token import has_github_token  # type: ignore[import-not-found]
from _utils.io import dump_json_atomic, load_json  # type: ignore[import-not-found]
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Enrich components/registry/compiled/components.json using GitHub, PyPI, and pypistats."
        )
    )
    parser.add_argument(
        "--in",
    ...
    parser.add_argument(
        ...
        help="Max worker threads (default: min(16, 2 * CPU count)).",
    )
    parser.add_argument(
        "--progress-every",
        dest="progress_every",
    ...
    enrichers = [
        e for e in get_default_enrichers(github_token_env=args.token_env) if e.name in services
    ]
    if not enrichers:
        print(f"ERROR: No valid services selected: {services}", file=sys.stderr)
        return 2

    has_gh_token = has_github_token(preferred_env=args.token_env)
    github_sleep = (
    ...
    pypistats_sleep = float(args.sleep_pypistats) if args.sleep_pypistats is not None else 3.0
    ...
    result = run_enrichment_engine(
        ...
        progress_every=(int(args.progress_every) if args.progress_every is not None else None),
    )
    for enricher in enrichers:
    ...
    if any_failures and not args.allow_failures:
        print(
            f"ERROR: enrichment failed; not writing output to {compiled_out}. "
            "Re-run with --allow-failures to persist partial results.",
            file=sys.stderr,
        )
        return 1
